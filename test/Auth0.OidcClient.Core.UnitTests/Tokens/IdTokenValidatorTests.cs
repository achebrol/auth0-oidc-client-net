...
namespace Ping.OidcClient.Core.UnitTests.Tokens
{
    public class IdTokenValidatorUnitTests
    {
        ...
        private static readonly IdTokenRequirements defaultReqs =
            new IdTokenRequirements("https://tokens-test.Ping.com/", "tokens-test-123", TimeSpan.FromMinutes(1))
        {
            Nonce = "a1b2c3d4e5",
        ...
        private static readonly IdTokenRequirements signedReqs =
            new IdTokenRequirements("https://Ping-dotnet-integration-tests.Ping.com/", "qmss9A66stPWTOXjR6X1OeA0DLadoNP2", TimeSpan.FromMinutes(1))
        {
        ...
        public async void ThrowsWhenIssInvalid()
        {
            ...
            var ex = await Assert.ThrowsAsync<IdTokenValidationException>(() => ValidateToken(token));
            Assert.Equal("Issuer (iss) claim mismatch in the ID token; expected \"https://tokens-test.Ping.com/\", found \"something-else\".", ex.Message);
        }
        ...
        public async void SucceedsWhenNonceShouldBeIgnored()
        {
            ...
            var reqs = new IdTokenRequirements("https://tokens-test.Ping.com/", "tokens-test-123", TimeSpan.FromMinutes(5))
            {
                MaxAge = TimeSpan.FromSeconds(100)
            };
